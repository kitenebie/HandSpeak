...
export async function mount(container, params) {
  ...
  container.innerHTML = `
    <div class="FSL-alphabet-page FSL-container">
      ...
      ${lessonsToShow.length ? `<section class="FSL-teacher-lessons"><div class="FSL-section__heading"><div><span class="FSL-eyebrow">From your teachers</span><h2>Learning activities</h2></div></div><div class="FSL-dashboard-grid">${lessonsToShow.map(lesson => `<article class="FSL-card FSL-teacher-lesson ${selectedLesson?.id === lesson.id ? 'FSL-teacher-lesson--selected' : ''}"><span class="FSL-classroom-activity__icon"><i data-lucide="book-open"></i></span><h3>${escape(lesson.title)}</h3><small>${escape(lesson.classrooms?.name || 'Classroom')}</small><p>${escape(lesson.description || 'Open the alphabet lesson and practice each sign.')}</p><button type="button" class="FSL-btn FSL-btn--secondary open-alphabet">Open alphabet <i data-lucide="arrow-down"></i></button></article>`).join('')}</div></section>` : ''}
