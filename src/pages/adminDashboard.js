...
function renderApexChart(container, selector, { labels, values, name, type = 'bar' }) {
  ...
  const options = {
    chart: { type, height: 300, toolbar: { show: false }, fontFamily: 'Inter, system-ui, sans-serif' },
    ...
    colors: ['#2563eb', '#14b8a6', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4', '#84cc16', '#f97316'],
...
function renderAdminDashboardCharts(container, { teachers, students, classrooms }) {
  ...
  renderApexChart(container, '#admin-population-gender-chart', {
    labels: genderLabels,
    values: genderKeys.map(gender =>
      teachers.filter(item => (item.gender || 'unspecified') === gender).length +
      students.filter(item => (item.gender || 'unspecified') === gender).length
    ),
    name: 'Users',
    type: 'donut'
  });
...
function filteredActivities(quizzes, teacherId) {
  return quizzes.filter(quiz => quiz.teacher_id === teacherId &&
    (!activityFilters.type || quiz.quiz_type === activityFilters.type) &&
    (!activityFilters.status || (activityFilters.status === 'published' ? quiz.is_published : !quiz.is_published)));
}

function handledStudents(data, teacherId) {
  const rooms = new Set(data.classrooms.filter(room => room.teacher_id === teacherId).map(room => room.id));
  const ids = new Set(data.memberships.filter(member => rooms.has(member.classroom_id)).map(member => member.student_id));
...
function renderActivityCards(data) {
  ...
  const teachers = data.teachers.filter(teacher =>
    `${teacher.full_name || ''} ${teacher.email || ''}`.toLowerCase().includes(search) &&
    (!(activityFilters.type || activityFilters.status) || filteredActivities(data.quizzes, teacher.id).length));
  return `<p class="FSL-muted" role="status">${teachers.length} teacher${teachers.length === 1 ? '' : 's'} found</p><section class="FSL-activity-grid">${teachers.map(teacher => {
  ...
  }).join('') || '<div class="FSL-empty-card">No teachers match these filters.</div>'}</section>`;
}
...
function render(container, data) {
  ...
  const openDeleteTeacherModal = (teacher, trigger) => {
    ...
    const cancel = backdrop.querySelector('.FSL-modal__actions [data-delete-cancel]');
    const dismiss = () => {
      if (deleting) return;
      closeModal();
    ...
    backdrop.addEventListener('click', event => {
      if (event.target === backdrop || event.target.closest('[data-delete-cancel]')) dismiss();
    });
    ...
    confirm.addEventListener('click', async () => {
      ...
      try {
        ...
        pendingAdminMessage = { text: 'Teacher account and invitation deleted.', type: 'success' };
        closeModal();
        await mount(container);
      } catch (error) {
        deleting = false;
        backdrop.querySelector('[data-delete-error]').textContent = error.message || 'Could not delete teacher.';
  ...
  const openTeacherModal = (teacher = null) => {
    ...
    form.addEventListener('submit', async event => {
      ...
      try {
        ...
        pendingAdminMessage = { text: teacher ? 'Teacher record updated.' : 'Teacher invitation sent.', type: 'success' };
        closeModal();
        await mount(container);
      } catch (error) {
        message.className = 'FSL-form__message FSL-form__message--error';
        message.textContent = error.message || (teacher ? 'Could not update teacher.' : 'Could not register teacher.');
  ...
  const openActivityDrawer = (teacherId, showStudents = false) => {
    ...
    drawer.setAttribute('aria-label', showStudents ? 'Handled students' : 'Teacher activities');
    drawer.innerHTML = `
      ...
      <div class="FSL-info-list">${quizzes.map(quiz => `
        ...
        </article>`).join('') || '<p class="FSL-muted">No activities match the current filters.</p>'}</div>`;
    if (showStudents) {
      drawer.querySelector('.FSL-info-list').innerHTML = `<p>${students.length} handled students</p>${students.length ? '<p class="FSL-muted">Drag to pan. Use the toolbar to zoom or fit the tree.</p><div class="FSL-student-tree"></div>' : '<p class="FSL-empty-card">No students are assigned to this teacher yet.</p>'}`;
    }
    ...
    if (showStudents && students.length) {
      ...
      try {
        activeStudentTree = new ApexTree(target, {
          width: '100%', height: 520, nodeWidth: 210, nodeHeight: 86,
          direction: 'top', contentKey: 'data', enableToolbar: true,
          enableAnimation: false, childrenSpacing: 60, siblingSpacing: 18,
          nodeTemplate: content => `<div class="FSL-student-tree__node"><strong>${escape(content.name)}</strong><small>${escape(content.detail)}</small></div>`,
          canvasStyle: 'background: #f8fafc; border: 1px solid #e2e8f0;',
        });
        activeStudentTree.render({
          id: `teacher-${teacher.id}`, data: { name: teacher.full_name || teacher.email, detail: 'Teacher' },
          children: students.map(student => ({ id: `student-${student.id}`, data: { name: student.full_name || student.email, detail: student.email || genderLabel(student.gender) } }))
        });
      } catch (error) {
        destroyStudentTree();
        target.textContent = 'Could not display the student tree. Please reopen the drawer.';
      }
    }
    drawer.addEventListener('keydown', event => {
      if (event.key === 'Escape') closeDrawer();
      if (event.key === 'Tab') {
        const controls = [...drawer.querySelectorAll('button, input, select, a[href], [tabindex="0"]')].filter(el => !el.disabled);
  ...
  const refreshActivities = () => {
    container.querySelector('#admin-activity-results').innerHTML = renderActivityCards(data);
    createIcons({ icons });
  };
  ...
  filters?.addEventListener('reset', event => {
    ...
    for (const name of ['search', 'type', 'status']) filters.elements[name].value = '';
    refreshActivities();
  });
  ...
  const handleAdminAction = async event => {
    ...
    if (button.dataset.action === 'delete-teacher') {
      const teacher = data.teachers.find(item => item.id === button.dataset.id);
    ...
    if (button.dataset.action === 'view-handled-students') openActivityDrawer(button.dataset.id, true);
